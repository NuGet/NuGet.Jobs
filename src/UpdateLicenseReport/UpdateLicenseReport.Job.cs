...
namespace UpdateLicenseReport
{
    class Job : JobBase
    {
        ...
        {
            JobEventSourceLog.FetchingNextReportUrl(PackageDatabase.DataSource, PackageDatabase.InitialCatalog);
            ...
            using (var connection = await PackageDatabase.ConnectTo())
            {
                ...
                {
                    JobEventSourceLog.NoNextReportFound();
                }
                else if (!Uri.TryCreate(nextReportUrl, UriKind.Absolute, out nextLicenseReport))
                {
                    JobEventSourceLog.InvalidNextReportFound(nextReportUrl);
                }
                nextLicenseReport = nextLicenseReport ?? LicenseReportService;
            }
            JobEventSourceLog.FetchedNextReportUrl(PackageDatabase.DataSource, PackageDatabase.InitialCatalog, (nextLicenseReport == null ? String.Empty : nextLicenseReport.AbsoluteUri));
        ...
        private async Task<bool> ProcessReports(Uri nextLicenseReport)
        {
            ...
            int tries = 0;
            JobEventSourceLog.RequestingLicenseReport(nextLicenseReport.AbsoluteUri);
            while (tries < RetryCount.Value && response == null)
            {
                ...
                if (thrown != null)
                {
                    JobEventSourceLog.ErrorDownloadingLicenseReport(nextLicenseReport.AbsoluteUri, thrown.ToString());
                    await Task.Delay(10 * 1000);
                }
            }
            JobEventSourceLog.RequestedLicenseReport(nextLicenseReport.AbsoluteUri);

            JobEventSourceLog.ProcessingLicenseReport(nextLicenseReport.AbsoluteUri);
            using (response)
            {
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    JobEventSourceLog.ReadingLicenseReport(nextLicenseReport.AbsoluteUri);
                    string content;
                    using (var reader = new StreamReader(response.GetResponseStream()))
                    {
                        content = await reader.ReadToEndAsync();
                    }
                    JobEventSourceLog.ReadLicenseReport(nextLicenseReport.AbsoluteUri);

                    JObject sonatypeMessage = JObject.Parse(content);
                    if (!sonatypeMessage.IsValid(sonatypeSchema))
                    {
                        JobEventSourceLog.InvalidLicenseReport(nextLicenseReport.AbsoluteUri, "UpdateLicenseReportsJob_JsonDoesNotMatchSchema");
                        return false;
                    }
                    ...
                    for (int i = 0; i < events.Count; i++)
                    {
                        ...
                        JobEventSourceLog.StoringLicenseReport(report.PackageId, report.Version);

                        if (await StoreReport(report) == -1)
                        {
                            JobEventSourceLog.PackageNotFound(report.PackageId, report.Version);
                        }
                        else
                        {
                            JobEventSourceLog.StoredLicenseReport(report.PackageId, report.Version);
                        }
                    }
                    ...
                    if (sonatypeMessage["next"].Value<string>().Length > 0)
                    {
                        ...
                        {
                            JobEventSourceLog.InvalidNextReportUrl(nextReportUrl);
                            return false;
                        }
                        JobEventSourceLog.StoringNextReportUrl(nextLicenseReport.AbsoluteUri);
                    ...
                    JobEventSourceLog.ProcessedLicenseReport(nextLicenseReport.AbsoluteUri);
                }
                else if (response.StatusCode != HttpStatusCode.NoContent)
                {
                    JobEventSourceLog.NoReportYet(nextLicenseReport.AbsoluteUri);
                }
                else
                {
                    JobEventSourceLog.ReportHttpError(nextLicenseReport.AbsoluteUri, (int)response.StatusCode, response.StatusDescription);
                }
                return false;
            }
        }
