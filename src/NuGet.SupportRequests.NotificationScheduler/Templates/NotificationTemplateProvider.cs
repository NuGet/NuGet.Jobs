...
namespace NuGet.SupportRequests.NotificationScheduler.Templates
{
    internal static class NotificationTemplateProvider
    {
        ...
        internal static string Get(string name)
        {
            ...
            {
                var htmlTemplate = ReadTemplateFromResoureName($"{_templatesNamespace}.{name}");
                var cssStyles = ReadTemplateFromResoureName(_cssStylesResourceName);
        ...
        private static string ReadTemplateFromResoureName(string resourceName)
        {
            string template;
