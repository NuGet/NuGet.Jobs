...
namespace NuGet.Services.Validation
{
    public class ValidationResponse : IValidationResponse
    {
            ...
            string outputUrl)
        {
            ...
            {
                throw new ArgumentException("Cannot specify results if the validation is not in a terminal status.", nameof(status));
            }

            if(!string.IsNullOrEmpty(outputUrl) && status != ValidationStatus.Succeeded)
            {
                throw new ArgumentException("Cannot specify outputUrl if the validation is not in a success state.", nameof(status));
            }
