...
{
    public class PackageCompatibilityValidatorResult
    {
        public PackageCompatibilityValidatorResult(ValidationStatus state)
            : this(state, new IValidationIssue[0])
        {
        }

        public PackageCompatibilityValidatorResult(ValidationStatus state, IReadOnlyList<IValidationIssue> issues)
        {
            State = state;
