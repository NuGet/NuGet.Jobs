    ...
    public class MessageChangeEventIteratorTests
    {
        public class TheIterateMethod : MessageChangeEventIteratorTest
        {
            ...
            public async Task IteratesChangesInOrder()
            {
                ...
                var firstContext = new CurrentMessageContext(firstChange.Timestamp, firstComponent, ComponentStatus.Degraded);
                Processor
                    .Setup(x => x.Process(firstChange, eventEntity, root, null))
                    .ReturnsAsync(firstContext)
                    .Verifiable();

                var secondComponent = new TestComponent("second");
                var secondContext = new CurrentMessageContext(secondChange.Timestamp, secondComponent, ComponentStatus.Degraded);
                Processor
                    .Setup(x => x.Process(secondChange, eventEntity, root, firstContext))
                    .ReturnsAsync(secondContext)
                    .Verifiable();

                var thirdComponent = new TestComponent("third");
                var thirdContext = new CurrentMessageContext(thirdChange.Timestamp, thirdComponent, ComponentStatus.Degraded);
                Processor
                    .Setup(x => x.Process(thirdChange, eventEntity, root, secondContext))
                    .ReturnsAsync(thirdContext)
                    .Verifiable();

                await Iterator.Iterate(changes, eventEntity);
                ...
                Processor
                    .Verify(
                        x => x.Process(
                            ...
                            It.IsAny<IComponent>(),
                            It.IsAny<CurrentMessageContext>()),
                        Times.Exactly(3));
            }
