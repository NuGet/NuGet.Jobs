...
namespace StatusAggregator.Tests.Collector
{
    public class ManualStatusChangeCollectorProcessorTests
    {
        ...
        public abstract class TheFetchSinceMethod : ManualStatusChangeCollectorTest
        {
            ...
            {
                Table
                    .Setup(x => x.CreateQuery<ManualStatusChangeEntity>())
                    .Returns(Enumerable.Empty<ManualStatusChangeEntity>().AsQueryable());
            ...
            public async Task HandlesManualChanges()
            {
                ...
                var changes = new[] { secondChange, firstChange };
                Table
                    .Setup(x => x.CreateQuery<ManualStatusChangeEntity>())
                    .Returns(changes.AsQueryable());
                ...
                Assert.Equal(changes.Max(c => c.Timestamp).UtcDateTime, result);
